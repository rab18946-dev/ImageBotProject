...
def process_image(input_path, text1, text2, index, logo_position="top_left"):
    ...
    # חישוב נתונים לכל שורה למרכוז מדויק
    line_data = []
    max_text_width = 0
    for line in lines:
    ...
    # --- מרכוז הטקסט בתוך התיבה הלבנה ---
    # מחשבים את נקודת ההתחלה הכללית שתהיה במרכז התיבה
    current_y = y1 + (box_height - total_text_height) // 2

    for d in line_data:
        # מרכוז אופקי (X)
        tx = (width - d['width']) // 2
        # ציור הטקסט עם ביטול ה-offset של הפונט למרכוז מושלם
...
# --- שאר קוד ה-HTML וה-Flask נשארים ללא שינוי ---
HTML = """
<!DOCTYPE html>
<html lang="he">
<head>
...
function addRow(){
    ...
    row.innerHTML = `
        ...
        <select><option value="top_left">שמאל למעלה</option><option value="top_right">ימין למעלה</option></select>
